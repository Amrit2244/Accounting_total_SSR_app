...
import {
  ...
  PlusCircle,
  ExternalLink,
  Calendar as CalendarIcon, // Renamed to avoid confusion
} from "lucide-react";
import confetti from "canvas-confetti";
import { format, isValid } from "date-fns"; // ✅ Using date-fns for formatting
...
export default function VoucherForm({
  ...
  if (state?.success) {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[500px] text-center p-8 bg-white border border-slate-200 rounded-3xl shadow-xl animate-in zoom-in-95">
        <div
        ...
        <button
          ...
          className="px-6 py-3 bg-slate-900 text-white rounded-xl font-bold text-xs uppercase hover:bg-indigo-600 transition-colors mt-4"
        >
          Create Another
        </button>
      </div>
  ...
  return (
    ...
    >
      ...
      {/* ✅ The hidden date input carries the ISO value for the database */}
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
        ...
        <div className="md:col-start-4">
          <label className="flex items-center justify-center gap-2 h-10 px-4 bg-white border border-slate-200 rounded-lg cursor-pointer hover:border-indigo-300 hover:text-indigo-600 text-slate-500 text-[10px] font-black uppercase transition-all shadow-sm">
      ...
      {/* GRID SECTION */}
      <div className="bg-white border border-slate-200 rounded-xl shadow-lg flex flex-col flex-1">
        <div className="grid grid-cols-12 bg-slate-900 text-white p-3 text-[10px] font-black uppercase tracking-widest rounded-t-xl">
          <div className="col-span-2 text-center">Type</div>
          <div className="col-span-8 pl-2">Particulars</div>
          <div className="col-span-2 text-right pr-4">Amount</div>
        </div>

        <div className="flex-1 p-2 space-y-1 bg-slate-50/50 min-h-[200px]">
          {rows.map((row, idx) => (
            <div
              key={idx}
            ...
            >
              ...
              <input
                ...
                className="col-span-2 h-9 px-3 border border-slate-200 rounded-lg text-xs font-mono font-bold text-right outline-none bg-white focus:ring-2 focus:ring-indigo-500/20"
                required
              />
              {rows.length > 2 && (
                <button
                  ...
                  className="absolute -right-1 opacity-0 group-hover:opacity-100 p-1 text-rose-500 hover:bg-rose-100 rounded"
                >
                  <Trash2 size={10} />
                </button>
              )}
        ...
        <button
          ...
          className="w-full py-3 bg-white text-[10px] font-black text-slate-500 border-t border-slate-200 hover:text-indigo-600 rounded-b-xl uppercase"
        >
      ...
      <div className="flex flex-col md:flex-row gap-6 items-start">
        ...
        />
        <div
          className={`w-full md:w-72 text-white p-5 rounded-2xl flex flex-col justify-between h-36 shadow-xl ${
        ...
        >
          <div>
            <div className="flex justify-between text-indigo-200 text-[10px] font-bold">
              <span>Dr:</span>{" "}
              <span className="text-white">{totalDr.toFixed(2)}</span>
            </div>
            <div className="flex justify-between text-orange-200 text-[10px] font-bold">
              <span>Cr:</span>{" "}
              <span className="text-white">{totalCr.toFixed(2)}</span>
            </div>
          </div>
          <div className="flex justify-between items-end pt-2">
            ...
            <button
              ...
              className="h-9 px-4 bg-white text-slate-900 rounded-lg text-[10px] font-black uppercase hover:bg-indigo-50 disabled:opacity-50 flex items-center gap-2"
            >
              {isPending ? (

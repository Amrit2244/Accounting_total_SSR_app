...
export default async function VoucherDetailsPage({
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-slate-900 selection:bg-indigo-100 selection:text-indigo-700 flex flex-col">
      ...
      <div className="relative z-10 max-w-4xl mx-auto p-6 md:p-8 space-y-6 flex-1 w-full">
        {/* HEADER BAR */}
        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
          <div className="bg-slate-50/50 p-6 grid grid-cols-2 md:grid-cols-4 gap-6 border-b border-slate-100">
            <MetadataItem
              label="Date"
              value={format(new Date(voucher.date), "dd MMM yyyy")}
            />
            <MetadataItem
              label="Amount"
              value={`₹${(voucher.totalAmount || 0).toLocaleString("en-IN")}`}
              isMono
            />
          ...
          <div className="p-6">
            <h3 className="text-xs font-black text-slate-900 uppercase tracking-widest mb-4 flex items-center gap-2">
              <Layers size={14} className="text-indigo-600" /> Account Postings
            </h3>
          ...
          {voucher.narration && (
            <div className="px-6 pb-6 pt-2">
              <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl text-slate-700 text-xs flex gap-3 items-start italic leading-relaxed">
